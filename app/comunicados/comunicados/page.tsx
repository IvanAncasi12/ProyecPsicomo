...
export default function ComunicadosOficialesPage() {
  ...
  function esComunicado(item: any) {
    ...
    return tipoId === 4 || tipoNombre === 'COMUNICADOS'
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const soloComunicados = convs.filter((comunicado: any) =>
          esComunicado(comunicado)
        )

        const formatted = soloComunicados.map((comunicado: any) => ({
          ...comunicado,
          img_url: buildImageUrl(comunicado.con_foto_portada),
        }))
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-base md:text-lg text-slate-600 max-w-3xl mx-auto font-medium leading-relaxed mt-5">
              Comunicados institucionales emitidos por la carrera para informar
              disposiciones, actividades y novedades oficiales.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-3xl border border-dashed border-slate-300 bg-white shadow-sm">
              ...
              <p className="text-slate-600 font-medium">
                Cuando se registren comunicados en el servicio, aparecerán aquí
                automáticamente.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              {comunicados.map((comunicado, index) => {
                const titulo =
                  cleanValue(comunicado.con_titulo) ||
                  `Comunicado ${index + 1}`

                const descripcion =
                  cleanHtml(comunicado.con_descripcion) ||
                ...
                return (
                  <article
                    key={comunicado.idconvocatorias || index}
                    ...
                    <div className="relative flex flex-col md:flex-row gap-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-3 mb-3">
                          ...
                          <span
                            className="inline-flex px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-[0.12em] text-white"
                            style={{
                              background: `linear-gradient(135deg, ${colorPrimario}, ${colorSecundario})`,
                            }}
                          >
                        ...
                        <div className="grid sm:grid-cols-2 gap-3">
                          <InfoBox
                            icon="📅"
                            label="Publicado"
                            value={formatDate(comunicado.con_fecha_inicio)}
                          />

                          <InfoBox
                            icon="🏁"
                            label="Vigencia"
                            value={formatDate(comunicado.con_fecha_fin)}
                          />
                        </div>
                      </div>
                    </div>
