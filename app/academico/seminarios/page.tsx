  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const seminariosFiltrados = todosCursos.filter(
          (curso: any) =>
            curso.tipo_curso_otro?.tipo_conv_curso_nombre?.toUpperCase() ===
            'SEMINARIOS'
        )

        const formatted = seminariosFiltrados.map((seminario: any) => ({
          ...seminario,
          img_url: buildImageUrl(seminario.det_img_portada),
        }))

        setSeminarios(formatted)
      } catch (error) {
        console.error('Error cargando seminarios:', error)
      } finally {
        setLoading(false)
      }
    }
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            >
              ...
              />
              Espacios de análisis jurídico
            </span>

            <p className="text-base md:text-lg text-slate-600 max-w-3xl mx-auto font-medium leading-relaxed mt-5">
              Actividades académicas orientadas al debate, reflexión y
              actualización en temas jurídicos, sociales e institucionales.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-3xl border border-dashed border-slate-300 bg-white shadow-sm">
              ...
              <p className="text-slate-600 font-medium">
                Cuando se registren seminarios en el servicio, aparecerán aquí
                automáticamente.
              </p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {seminarios.map((seminario, index) => {
                const titulo =
                  cleanValue(seminario.det_titulo) || `Seminario ${index + 1}`

                const descripcion =
                  cleanHtml(seminario.det_descripcion) ||
                  'Sin descripción disponible.'

                const costo = cleanValue(seminario.det_costo)
                const lugar = cleanValue(seminario.det_lugar_curso)
                const hora = cleanValue(seminario.det_hora_ini)

                return (
                  <article
                    key={seminario.iddetalle_cursos_academicos || index}
                    ...
                    <div className="relative p-6">
                      ...
                      <div className="space-y-3 rounded-2xl bg-slate-50 border border-slate-100 p-4">
                        <InfoRow
                          icon="📅"
                          label="Fecha"
                          value={formatDate(seminario.det_fecha_ini)}
                        />
