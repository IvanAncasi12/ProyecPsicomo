...
export default function TalleresPage() {
  ...
  function esTaller(evento: any) {
    const titulo = String(evento?.evento_titulo || '').toLowerCase()
    const tipo = String(evento?.tipo_evento || '').toLowerCase()
    const descripcion = String(evento?.evento_descripcion || '').toLowerCase()

    return (
      titulo.includes('taller') ||
      titulo.includes('congreso') ||
      titulo.includes('conferencia') ||
      titulo.includes('seminario') ||
      tipo.includes('taller') ||
      tipo.includes('congreso') ||
      tipo.includes('conferencia') ||
      tipo.includes('seminario') ||
      descripcion.includes('taller') ||
      descripcion.includes('congreso') ||
      descripcion.includes('conferencia') ||
      descripcion.includes('seminario')
    )
  }

  function getTipoTaller(evento: any) {
    const texto = `${evento?.evento_titulo || ''} ${evento?.tipo_evento || ''}`
      .toLowerCase()

    if (texto.includes('congreso')) return 'Congreso'
    if (texto.includes('conferencia')) return 'Conferencia'
    if (texto.includes('seminario')) return 'Seminario'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const talleresFiltrados = todosEventos.filter((evento: any) =>
          esTaller(evento)
        )

        const formatted = talleresFiltrados.map((evento: any) => ({
          ...evento,
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-base md:text-lg text-slate-600 max-w-3xl mx-auto font-medium leading-relaxed mt-5">
              Talleres, congresos, conferencias y espacios de capacitación
              orientados al fortalecimiento académico y profesional.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 rounded-3xl border border-dashed border-slate-300 bg-white shadow-sm">
              ...
              <p className="text-slate-600 font-medium">
                Cuando se registren talleres, congresos o conferencias en el
                servicio, aparecerán aquí automáticamente.
              </p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {talleres.map((taller, index) => {
                const titulo =
                  cleanValue(taller.evento_titulo) || `Actividad ${index + 1}`

                const descripcion =
                  cleanHtml(taller.evento_descripcion) ||
                  'Sin descripción disponible.'

                const lugar = cleanValue(taller.evento_lugar) || 'Por definir'
                const tipo = getTipoTaller(taller)

                return (
                  <article
                    key={taller.evento_id || index}
                    ...
                    <div className="relative p-6">
                      ...
                      <div className="space-y-3 rounded-2xl bg-slate-50 border border-slate-100 p-4">
                        <InfoRow
                          icon="📅"
                          label="Fecha"
                          value={formatDate(taller.evento_fecha)}
                        />

                        <InfoRow
                          icon="⏰"
                          label="Hora"
                          value={formatTime(taller.evento_hora)}
                        />
